...
## corpus (the Tier A validation set; generated, never shipped as media)

- corpus-manifest.yaml: the thirteen named items with a digest of each one's analytic render and its development or held-out split. Shipped rather than recomputed so a dependency change surfaces as a failed test instead of silently moving every number. Regenerate only when the catalogue changes on purpose, with tools/write_corpus_manifest.py.
- CITATIONS.yaml: the citation ledger. Every [S:], [M:], and [C:] tag in this directory is either verified here or named in pending_backfill with a reason, enforced offline by tests/test_citations.py.
